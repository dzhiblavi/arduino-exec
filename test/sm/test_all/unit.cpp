...
TEST_F(t_all, test_all_no_child_continuations) {
    Task t1({});
    Task t2({});
...
TEST_F(t_all, test_all_with_child_continuations) {
    Callback cb;
...
TEST_F(t_all, test_all_installs_cancellations) {
...
TEST_F(t_all, test_all_both_complete) {
    CancellationSignal s;
    Callback cb;
    Task t1({});
    Task t2({});
    ...
    a(t1(), t2())(&cb, s.slot());
    TEST_ASSERT_EQUAL(noop, t1.init_runnable->run());
    TEST_ASSERT_TRUE(s.hasHandler());
    TEST_ASSERT_FALSE(t1.cancel_called || t2.cancel_called);
    TEST_ASSERT_EQUAL(&cb, t2.init_runnable->run());
    TEST_ASSERT_FALSE(t1.cancel_called || t2.cancel_called);
    TEST_ASSERT_FALSE(s.hasHandler());
}

TEST_F(t_all, test_all_neither_complete) {
    CancellationSignal s;
    Callback cb;
    Task t1({});
    Task t2({});
...
TEST_F(t_all, test_all_one_completes) {
    CancellationSignal s;
    Callback cb;
    Task t1({});
    Task t2({});
